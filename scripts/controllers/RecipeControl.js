...
class Recipe {
   ...
   * @return {string} appareil de la recette.
   */
  getAppliance() {
   ...
   * @return {number} L'ID de la recette.
   */
  getId() {
    return this.ID;
  }
   ...
   * @return {Array} Lingrédients de la recette.
   */
  getIngredients() {
   ...
   * @return {number} nombre de portions de la recette.
   */
  getServings() {
   ...
   * @return {number} temps de préparation de la recette.
   */
  getTime() {
   ...
   * @return {Array} Les ustensiles de la recette.
   */
  getUstensils() {
   ...
   * @return {HTMLElement} La carte de la recette.
   */
  getCard() {
...
function summarize() {
  ...
  const {length} = NumberOfCards;
  // Récuère le nombre d'éléments du tableau recipesArray.
  const TagRecipes = length > 1 ? `${length} recettes` : `${length} recette`;
  resume.innerHTML = '';
  resume.innerHTML = TagRecipes;
  // Affichage de longueur du tableau recipesArray.
}
...
function displayRecipes(Array) {
  ...
  for (let i = 0; i < Array.length; i += 1) {
    // Lit le tableau recipesArray
    // création d'une carte de recette pour chaque élément.
    const {
      appliance,
      description,
      id,
